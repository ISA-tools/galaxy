...
def add_file(dataset, registry, json_file, output_path):
    ...
    if dataset.type == 'url':
        try:
            ...
            temp_name, dataset.is_multi_byte = sniff.stream_to_file(page, prefix='url_paste',
        ...
        if getattr(root_datatype, 'compressed', False):
            ...
            if is_gzipped and not is_valid:
                ...
                if link_data_only == 'copy_files':
                    ...
                    CHUNK_SIZE = 2 ** 20  # 1Mb
                    fd, uncompressed = tempfile.mkstemp(prefix='data_id_%s_upload_gunzip_' % dataset.dataset_id,
            ...
            if not data_type and bz2 is not None:
                ...
                if is_bzipped and not is_valid:
                    ...
                    if link_data_only == 'copy_files':
                        ...
                        CHUNK_SIZE = 2 ** 20  # 1Mb
                        fd, uncompressed = tempfile.mkstemp(prefix='data_id_%s_upload_bunzip2_' % dataset.dataset_id,
                                                            dir=os.path.dirname(output_path), text=False)
                    ...
                    if link_data_only == 'copy_files':
                        ...
                        for name in z.namelist():
                            ...
                            if unzipped:
                                ...
                                break
                            fd, uncompressed = tempfile.mkstemp(prefix='data_id_%s_upload_zip_' % dataset.dataset_id,
                                                                dir=os.path.dirname(output_path), text=False)
                ...
                if link_data_only == 'copy_files':
                    ...
                    if dataset.to_posix_lines:
                        ...
                        if dataset.space_to_tab:
                            line_count, converted_path = sniff.convert_newlines_sep2tabs(dataset.path,
                                                                                         in_place=in_place,
                                                                                         tmp_dir=tmpdir,
                                                                                         tmp_prefix=tmp_prefix)
                        else:
                            line_count, converted_path = sniff.convert_newlines(dataset.path, in_place=in_place,
                                                                                tmp_dir=tmpdir, tmp_prefix=tmp_prefix)
